...
class PubNub(object):
        ...
        class MySubscribeCallback(SubscribeCallback):
            ...
            def message(self, pubnub, message):
                encryptionKey = base64.b64decode(
                    this.subscription['deliveryMode']['encryptionKey'])
                aes = AES.new(encryptionKey, AES.MODE_ECB)
                json_string = aes.decrypt(base64.b64decode(
                    message.message)).decode('utf-8').strip()
                message_callback(json_string)
    ...
    def refresh(self):
        if not self.subscription:
            return
        r = self.rc.put('/restapi/v1.0/subscription/{id}'.format(
    ...
    def revoke(self):
        if not self.subscription:
            return
        self.pubnub.unsubscribe().channels(
            self.subscription['deliveryMode']['address']).execute()
        self.pubnub.remove_listener(self.callback)
        self.pubnub = None
        self.rc.delete('/restapi/v1.0/subscription/{id}'.format(
            id=self.subscription['id']))
